...
The domain tag is plainly prefixed as in NUT-10. A BAT is not a transaction input, so it derives no `input_id` or `input_digest`. The BAT itself does not appear in the transcript (the witness signature binds it).

The serialized `AuthProof` carries a required `witness`, the [key path](10.md#key-path) shape: one BIP-340 signature over `request_digest` by the secret's key. The mint MUST reject a version `02` BAT whose witness is missing or invalid.

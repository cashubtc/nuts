...
The transcript holds exactly one authorized-request container (`0x05`): `01` the HTTP method, `02` the request-target, `03` the body hash, encoded as [NUT-10](10.md#the-transaction-transcript) defines them. The BAT itself does not appear in the transcript (the witness signature binds it).

The serialized `AuthProof` carries a required `witness`, the [key path](10.md#key-path) shape: one BIP-340 signature over the transcript digest by the secret's key. The mint MUST reject a version `02` BAT whose witness is missing or invalid.

...
On keysets with version byte `00` or `01`, wallets MUST validate the DLEQ proofs `dleq` as defined in [NUT-12](./12.md) to prevent pinning; should `AuthProofs` be sent to another user of the mint, they MUST include the `dleq` proof so that the receiving user can validate it.

On keysets with version byte `02` there is no `dleq`: wallets verify `C` by pairing ([NUT-12](./12.md)).
...
On keysets with version byte `00` or `01`, the mint MUST return DLEQ proofs for every blind signature in `PostAuthBlindMintResponse` as defined in [NUT-12](./12.md). On keysets with version byte `02` no `dleq` is returned ([NUT-12](./12.md)).

...
To prevent pinning, wallets MUST verify the mint's signature on every `AuthProof`: on keysets with version byte `02` or later by the [pairing equality](00.md#pairing-based-bls-blind-signatures), and on earlier keysets by the `dleq` proof ([legacy NUT-12](legacy/12.md)). A pre-v3 `AuthProof` sent to another user of the mint MUST include its `dleq` so that the receiving user can validate it.
...
## Nutroot blind authentication (v3 keysets)

On an auth keyset with version byte `02`, a BAT's `secret` is a [nutroot point secret](10.md#nutroot-secrets-v3-keysets): the compressed public key of a fresh random keypair per BAT, whose private key the wallet keeps. A BAT is never a transaction input (it cannot be swapped and appears only in the `Blind-auth` header); it signs the request it authorizes instead, as a **request transcript** under the one [transaction transcript](10.md#the-transaction-transcript) construction.

The transcript holds exactly one authorized-request container (`0x05`): `01` the HTTP method, `02` the request-target, `03` the body hash, encoded as [NUT-10](10.md#the-transaction-transcript) defines them. Its digest is:

```
request_digest = SHA256(authorized_request_container)
message        = tagged_hash("Cashu_AuthorizedRequest", request_digest)
```

`tagged_hash` is [NUT-10](10.md#nutroot-secrets-v3-keysets)'s. A BAT is not a transaction input, so it derives no `input_id` or `input_digest`: `request_digest` is the request's plain content hash and `message` the bytes the witness signs. The BAT itself does not appear in the transcript (the witness signature binds it).

The serialized `AuthProof` carries a required `witness`, the [key path](10.md#key-path) shape: one BIP-340 signature over `message` by the secret's key. The mint MUST reject a version `02` BAT whose witness is missing or invalid.

The BAT travels in the `Blind-auth` header, outside the signed bytes, so no request canonicalization exists: an intermediary MUST relay the body of a blind-auth request byte for byte. See the [test vectors][tests] for a worked request transcript.
...
On pre-v3 keysets, the mint MUST return a DLEQ proof for every blind signature in `PostAuthBlindMintResponse` ([legacy NUT-12](legacy/12.md)). Keysets with version byte `02` or later carry no `dleq`: their signatures are verified by the [pairing equality](00.md#pairing-based-bls-blind-signatures).

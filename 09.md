...
The returned arrays `outputs` and `signatures` are of the same length and for every entry `B_s[i]`, there are corresponding `outputs[i]` and `signatures[i]` entries,
whith `outputs[i].B_ == B_s[i]`. 

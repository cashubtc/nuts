...
Cashu uses a blind BLS signatures scheme on BLS12-381. To generate the ecash secrets and to sign transactions, Schnorr signatures on secp256k1 are used.
...
We use the libsecp256k1 curve to generate the ecash secret public key `P` and its private signing key `p`
- `Gp` libsecp256k1 generator point
...
Compressed point encoding follows the standard BLS12-381 for points on `G1` and `G2` and libsecp256k1 serialisation for points on `Gp`. Hex encodings in JSON fields are lowercase, with no `0x` prefix and the fixed widths above. The identity point (point at infinity) is never a valid blinded message, signature, or public key.
...
- `p` secp256k1 private key for signing inputs
- `P = SecpPubKey(p)` compressed secp256k1 pubkey, corresponds to point `Y = hash_to_curve_G1(P)` in `G1`
...
- `Alice` picks `p`, generates `P = p·G` and computes `Y = hash_to_curve_G1(P)`
...
Implementations **MUST** deserialise `B_`, `C_`, `C`, and `K` values using the canonical compressed BLS12-381 encoding, and **MUST** reject:
...
`amount` is the amount of the `Proof`, `secret` is a 33-byte compressed secp256k1 pubkey in 66 character hex (or a [NUT-10][10] script), `C` is the unblinded BLS signature on `secret` (hex string), `id` is the [keyset id][02] of the mint public keys that signed the token (hex string).

`witness` carries a serialized 64 byte Schnorr signature from the pubkey `secret` and its corresponding private key `xpriv` over the serialized transaction, as defined in the next section. 

## 0.2 - Protocol

### Transaction signatures
To spend a `Proof` in a melt or a swap transaction, wallets **MUST** provide a Schnorr signature in the `witness`. The signature is over the entire transaction's canonical serialization. 

The canonical serialization format for swap and melt transactions that spend inputs is defined in the respective NUTs that define those transaction types, see [NUT-03][03] and [NUT-05][05]. The serialized transaction `msg_to_sign` string is then hashed and signed by `xpriv` for each input `Proof`.

> [!NOTE]
> The serialization for each transaction must deep-omit all `witness` fields for all inputs.

```
digest    = SHA-256(utf8(msg_to_sign))
domain    = bytes `b"CashuV3_Transaction_"`
signature = SchnorrSign(xpriv, domain || digest)
```

Here, `||` denotes byte concatenation. Verification of the Schnorr signature takes place with `secret = SecpPubKey(xpriv)` as the public key. Mints **MUST** refuse to process transactions if the signature validation fails for any of the transaction's inputs.
...
`s` denotes `secret_xpriv` for keyset v3 Proofs. Tokens from deprecated v1 and v2 keysets use `s` for `secret`.

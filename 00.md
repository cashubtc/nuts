...
> [!IMPORTANT]
> Rejection sampling **MUST** be used to avoid bias. `BLS_FR_ORDER ≈ 0.45 · 2^256`, so each iteration succeeds with probability ≈ 0.45 (about 2.2 expected iterations per proof). This yields `r_i` uniformly distributed over `Fr*`. Plain `mod BLS_FR_ORDER` would over-represent some values by ~36%.

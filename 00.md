...
Cashu uses a blind BLS signature scheme on BLS12-381. Ecash secrets are secp256k1 public keys, and transactions are signed with Schnorr signatures over that curve ([NUT-10][10]). The superseded Blind Diffie-Hellmann key exchange (BDHKE) on secp256k1 is specified in [legacy NUT-00](legacy/00.md).
...
- `BLS_FR_ORDER` order of the BLS12-381 prime-order subgroup (and of `Fr`)
- `Fr` finite field of integers modulo `BLS_FR_ORDER`; scalars live here.
- `G1`, `G2` BLS12-381 prime-order groups of order `BLS_FR_ORDER` (`Y`, `B_`, `C_`, `C` live in `G1`; `K` lives in `G2`); `G1` points come from `hash_to_curve_G1`.
- `g1`, `g2` fixed generators of `G1`, `G2`
...
Compressed point encoding follows the standard BLS12-381 serialisation. Hex encodings in JSON fields are lowercase, with no `0x` prefix and the fixed widths above. The identity point (point at infinity) is never a valid blinded message, signature, or mint key.
...
- `P` secret, a compressed secp256k1 public key (see [Secret bytes](#secret-bytes)), corresponds to point `Y = hash_to_curve_G1(P)` in `G1`
...
- `Alice` picks secret `P` and computes `Y = hash_to_curve_G1(P)`
...
- `Alice` can take the pair `(P, C)` as a token and send it to `Carol`.
- Anyone holding `K` can verify a proof `(P, C)` (**verification**) by computing `Y = hash_to_curve_G1(P)` and accepting only if `e(C, g2) == e(Y, K)`.
- `Carol` can send `(P, C)` to `Bob`, who treats it as a valid spend of a token, adding `P` to the list of spent secrets, if the token verifies and has not been seen before.

### `hash_to_curve_G1(P: bytes) -> G1 point Y`

Maps `P` deterministically to a point in `G1` using the BLS12-381 G1 random-oracle suite defined in [RFC 9380](https://www.rfc-editor.org/rfc/rfc9380) §8.8.1 (`BLS12381G1_XMD:SHA-256_SSWU_RO_`), with Cashu domain separation tag (DST):
...
For keysets with version byte `02` or later, `Proof.secret` is a compressed secp256k1 public key encoded as 66 lowercase hex characters ([NUT-10 Nutroot secrets](10.md#nutroot-secrets-v3-keysets)), and `P` is its **decoded 33 bytes**.

Mints **MUST** reject any other secret form on these keysets. The two generations' `Y` spaces cannot overlap regardless of the bytes: pre-v3 secrets hash to secp256k1 and v3 secrets to BLS12-381 G1.
...
For a batch of `n` proofs, each with mint key `K_i`, unblinded signature `C_i`, and secret bytes `secret_i` (the proof's [secret bytes](#secret-bytes), the same bytes fed to `hash_to_curve_G1`):
...
> [!IMPORTANT]
> Rejection sampling **MUST** be used to avoid bias.
...
Implementations **SHOULD** evaluate `Π_k e( Σ_{i:K_i=K_k} w_i·Y_i , K_k )` with a single call to a multi-miller loop primitive, at the end of which they **SHOULD** perform the final exponentiation over the accumulated miller loop.
...
The weights are public and deterministic; security does not rely on their secrecy.
...
- points with cofactor components

> [!IMPORTANT]
> Full validation with cofactor rejection is required to defend against the [small-subgroup attack][lim-lee].

# Cashu conventions and models

## General notation and conventions

These specifications use elliptic curve public key cryptography over the field and curve parameters defined by [secp256k1](http://www.secg.org/sec2-v2.pdf).
...
- `SHA256(m)` denotes the SHA 256 hash of byte sequence `m`.
...
`amount` is the amount of the `Proof`, `secret` is the [nutroot secret][10] as 66 lowercase hex characters, `C` is the unblinded signature on `secret` (hex string), `id` is the [keyset id][02] of the mint public keys that signed the token (hex string). `witness` satisfies the secret and is present only while the `Proof` is spent as a transaction input ([NUT-10][10]); serialized tokens carry no witness.

`secret` **MUST** be unique per `Proof` across the entire mint: each secret maps to a curve point `Y`, and mints reject a duplicated `Y` as already spent.
...
`cashu` is the Cashu token prefix. `[version]` is a single `base64_urlsafe` character to denote the token format version. Version `B` is defined below; version `A` is deprecated ([legacy NUT-00](legacy/00.md#v3-tokens)).
...
```sh
cashu:cashuBpGF0gaJhaUgArSaMTR9YJm...
```
      ...
      "p": [ // proofs with this keyset ID
        {
          ...
          "s": str, // secret
          "c": bytes, // signature
          "d": { <optional> // DLEQ proof, pre-v3 keysets only
            "e": bytes,
            "s": bytes,
            "r": bytes
          },
          "w": str <optional>, // optional witness
          "si": { ... } <optional> // spend info for keyset v3 proofs
        },
        ...
      ]
    },
...
`si` carries what the next holder of a v3 keyset proof needs beyond the proof itself; its fields and rules are defined in [NUT-10's spend info](10.md#spend-info). It is fund-critical for locked proofs (even a key-path spend needs the tree), so wallets **MUST** preserve `si` when re-serializing proofs they are not spending, including implementations that do not otherwise read it.

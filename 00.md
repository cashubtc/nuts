...
- `xpriv` secp256k1 private key for signing inputs
- `x = PubKey(xpriv)` compressed secp256k1 pubkey, corresponds to point `Y = hash_to_curve_G1(x)` in `G1`
...
Implementations **MUST** deserialise `x` using the canonical compressed secp256k1 encoding, and `B_`, `C_`, `C`, and `K` values using the canonical compressed BLS12-381 encoding, and **MUST** reject:
...
`amount` is the amount of the `Proof`, `secret` is a 33-byte compressed secp256k1 pubkey, `C` is the unblinded BLS signature on `secret` (hex string), `id` is the [keyset id][02] of the mint public keys that signed the token (hex string).

`witness` carries a Schnorr signature of pubkey `secret` from its corresponding private key `xpriv` over the canonical request body, as defined in the next section.

## 0.2 - Protocol

### Transaction signatures
To spend a `Proof` in a melt or a swap transaction, wallets **MUST** provide a Schnorr signature in the `witness`. The signature is over the entire transaction's hashed canonical JSON serialization according to RFC 8785 JCS with the `witness` field removed:

```
normalizedPayload = requestPayload.exclude("witness")
canonical = JCS(normalizedPayload)
digest    = SHA-256(canonical)
domain_separator = constant byte string `b"Secp256k1_HashToCurve_Cashu_"`
signature = Sign(xpriv, domain_separator || digest)
```

Verification of the Schnorr signature takes place with `secret` as the public key. Mints **MUST** refuse to process transactions if the signature validation fails for any of the transaction's inputs.

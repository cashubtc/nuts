...
- `x` secret bytes (see [Secret bytes](#secret-bytes)), corresponds to point `Y = hash_to_curve_G1(x)` in `G1`
...
For keysets with version byte `02` or later, `Proof.secret` is a compressed secp256k1 public key encoded as 66 lowercase hex characters ([NUT-10 Taproot secrets](10.md#taproot-secrets-v3-keysets)), and `x` is its **decoded 33 bytes**.
...
`secret` **MUST** be unique per `Proof`, on every keyset version: blinding a repeated secret on the same keyset and amount reproduces the same `C`, so the second issuance re-creates an existing `Proof` and its value is lost.

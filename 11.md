...
To spend Proofs locked with `P2PK`, the spender needs to include signatures in the Proofs used as inputs for the spending operation.

### Message aggregation

The message to sign depends on the type of transaction. `msg_to_sign` is defined in [NUT-03][03] for swap transactions and in [NUT-05][05] for melt transactions.
...
Signatures are stored in `P2PKWitness` objects and are provided in either each `Proof.witness`. `P2PKWitness` is a serialized JSON string of the form

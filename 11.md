...
> [!NOTE]
> This NUT applies to keysets with version byte `00` or `01`. On keysets with version byte `02` or later, every secret is a public key and key locks, multisig, and refund conditions are expressed as [NUT-10 taproot secrets](10.md#taproot-secrets-v3-keysets) (`threshold` and `after` leaves); `SIG_ALL` and `sigflag` do not exist there.

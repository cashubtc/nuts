...
```
Here, `Secret.data` is the public key of the recipient of the locked ecash. We serialize this `Secret` to a string in `Proof.secret` and get a blind signature by the mint that is stored in `Proof.C` (see [NUT-06][06]]). 
...
To spend a token locked with `P2PK`, the spender needs to include signatures in the spent proofs. We use Schnorr signatures on the SHA256 hash of the message to sign. We sign the fields `Proof.secret` (as the inputs) and `BlindedMessage.B_` (if the outputs also require a signature).

An ecash spending operation like `/split` ([NUT-06][06]]) and `/melt` ([NUT-06][05]]) can have multiple inputs and outputs. The inputs are the `Proofs` provided in the `proofs` field and the outputs are the `BlindedMessages` in the `outputs` field in the request body (see `PostMeltRequest` in [NUT-05][05] and `PostSplitRequest` in [NUT-06][06]). If we have more than one input or output, we provide signatures in each `Proof` and `BlindedMessage` individually. 

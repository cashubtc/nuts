...
This `Secret` serialized to a string in `Proof.secret`. A signature on this string by the public key `Secret.data` is added to `Proof.witness.signatures` so that the `Proof` can be spent.

...
Each signer **MUST** derive the message to sign from the package contents itself, as described in [Message aggregation for `SIG_ALL`](#message-aggregation-for-sig_all). The package deliberately carries no precomputed digest: a signer that trusts one blindly would sign a message it has not verified against the transaction it is approving. This also keeps the format independent of the message aggregation scheme in use.
...
```json
{
  ...
  "witness": {
    "signatures": <Array[hex_str]>
  } // Optional, decoded P2PKWitness object
}
```

> [!NOTE]
>
> `witness` is the **JSON-decoded** [`P2PKWitness`](#witness-format) object, not the serialized JSON string carried in `Proof.witness`. A wallet assembling the final transaction MUST serialize it before placing it in `Proof.witness`, and MUST NOT put an already-serialized string here, or the witness ends up double-encoded. By contrast, `inputs[].secret` is carried **verbatim** as it appears in `Proof.secret` and MUST NOT be transformed in transit.

The package is serialized to a string by JSON-encoding the object with specific key ordering (see `SigAllSigningPackage` definition), converting to base64url, and prefixing with `sigallA`. The keys must be sorted as: `version`, `type`, `quote` (if present), `inputs`, `outputs`, `witness` (if present).

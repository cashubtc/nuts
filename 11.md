...
message_hash = tagged_hash("Cashu_SigAllSig_v1", message)
```
...
- `tagged_hash` uses the [BIP-340] construction `SHA256(SHA256(tag) || SHA256(tag) || message)`, where `tag` is encoded as UTF-8 and each inner SHA-256 result is its raw 32-byte digest. For this tag, `SHA256("Cashu_SigAllSig_v1") = c83c413c874b6f3da4c6310558d0d174c56f1a0a034023ae225ab3648e9626b3`.

The inputs are the `Proofs` in the transaction's `inputs` field. The outputs are the `BlindedMessages` in its `outputs` field: for a swap all requested outputs, for a melt the [NUT-08][08] blank outputs (zero output entries if absent). The 32-byte `message_hash` is signed directly with the Schnorr scheme in [Signature scheme](#signature-scheme), without a further SHA-256.

...
To facilitate multi-party signing for [`SIG_ALL`](#signature-flag-sig_all) transactions, wallets can exchange a `SigAllSigningPackage` containing the minimal data required to reconstruct the transaction message and produce signatures. It is a wallet-led transport format.
...
The [SIG_ALL swap example] would be represented as the following `SigAllSigningPackage`:

...
In the basic case, the mint requires one valid Schnorr signature in `Proof.witness.signatures` on `Proof.secret` by the public key in `Proof.Secret.data`. 

An example P2PK `Secret` is:
```json
[
"P2PK", 
  {
...
We serialize this `Secret` into a string in `Proof.secret` and get a blind signature by the mint that is stored in `Proof.C`. To spend this proof, a signature on the serialized `Secret` by the public key `Secret.data` is added to `Proof.witness.signatures`:
...
As a mental model, one can view the `secret` as analogous to a Bitcoin transaction's `scriptPubKey` and `signatures` as a `scriptSig` (or a witness).
...
### Tags
Additional and more complex spending conditions can be defined in the tags in `Proof.tags`. All tags are optional. Tags are arrays with two or more strings being `["key", "value1", "value2", ...]`.

...
For keysets with version byte `02` or later this BIP32 path is superseded: quote lock keys derive from [NUT-13][13] with `derivation_type_byte` `0x04`, which keeps the independent counter this section requires while hardening every step, since a v3 quote lock key may be handed over ([NUT-04](04.md#taproot-transactions-v3-keysets)).

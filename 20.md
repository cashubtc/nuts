...
For minting on keysets with version byte `02` or later, quote lock keys derive from the [NUT-13 V3 message](13.md#v3-message) with `derivation_type_byte` `0x04`, rejection sampled against `SECP256K1_N`. Every step is hardened, since a v3 quote lock key may be handed over ([NUT-04](04.md#taproot-transactions-v3-keysets)), which rules out a watch-only equivalent of the xpub below.

The message frames an **empty** `keyset_id_bytes` (`u32_BE(0)`, no bytes): a quote is requested before any keyset is chosen, and the outputs of the [mint request](04.md#taproot-transactions-v3-keysets) settle that afterwards.

...
> [!NOTE]
> This NUT's message format applies to keysets with version byte `00` or `01`. For keysets with version byte `02` or later, quote locks are mandatory and sign the quote input's [NUT-10 input digest](10.md#the-transaction-transcript) instead (see [NUT-04](04.md#nutroot-transactions-v3-keysets)); quote lock keys derive per [NUT-13](13.md#v3-secret-key).
...
> [!NOTE]
> This section applies to minting on keysets with version byte `01` or lower only. Later keysets derive quote lock keys as [above](#deterministic-quote-locking-key-derivation-v3-keysets).

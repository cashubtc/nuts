...
> [!NOTE]
> **Deprecated.** A wallet that uses deterministic quote lock keys derives them per [NUT-13][13] (`derivation_type_byte` `0x04`) and **MUST NOT** issue new quotes with keys from the path below; random keys remain permitted. The path is retained to recover keys that legacy wallets created.

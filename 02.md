...
A keyset `id` is an identifier for a specific keyset. It can be derived by anyone who knows the set of public keys of a mint. Wallets **SHOULD** compute the keyset `id` for a given keyset themselves to confirm that the mint is supplying the correct keyset ID (see below).
...
```python
def derive_keyset_id(keys: Dict[int, PublicKey], unit: str, final_expiry: Optional[int]) -> str:

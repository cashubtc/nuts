...
The keyset `id` is part of each `Proof` object which allows wallets to identify which mint and keyset it was generated from. The keyset field `id` is present in the `BlindedMessages` sent to the mint and in the `BlindSignatures` returned from it. It is also included in serialized Cashu `Tokens` (see [NUT-00][00]).

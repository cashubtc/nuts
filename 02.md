...
The one exception is change for overpaid Lightning fees ([NUT-08][08]). Blank outputs are checked against `active` keysets when the melt request is accepted, but are signed only after the payment settles. If their keyset has been inactivated in between, the mint **MUST** still sign the change with that keyset and **MUST NOT** substitute another. Swap and mint outputs have no such exception.

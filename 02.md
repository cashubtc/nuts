...
Mints can have multiple keysets at the same time but **MUST** have at least one `active` keyset (see [NUT-01][01]). The `active` property determines whether the mint allows generating new ecash from this keyset. `Proofs` from inactive keysets with `active=false` are still accepted as inputs but new outputs (`BlindedMessages` and `BlindSignatures`) **MUST** be from keysets that are `active` when the mint accepts the request.

Accepting a request bings outputs to their keyset: the mint **MUST** sign them with that keyset even if it is inactivated before signing completes, and **MUST NOT** substitute another. For swap and mint, acceptance and signing are a single step. For melt, blank change outputs ([NUT-08][08]) are accepted with the request but signed only after the payment settles.
...
### Keyset active window

A mint **MAY** announce when a keyset becomes active with `active_from`, and **MUST** announce when it intends to inactivate a keyset with `active_until`. Both are unix epoch timestamps. A mint **MUST NOT** inactivate a keyset that has no published `active_until`. Neither field is part of the keyset ID, so a mint can add or extend them without rotating.

- `active_from` announces a keyset ahead of time. The keyset is listed with `active=false` until that time, and wallets **MUST NOT** use it for outputs until the mint reports it as `active`.

- `active_until` is the earliest time the mint may inactivate the keyset. The mint **MUST NOT** inactivate the keyset before `active_until` and **MUST NOT** move `active_until` earlier once published. It **MAY** be any time when the keyset is first published. If it is added or changed later, it **MUST** be at least 30 days in the future. Where set, the timestamps **MUST** satisfy `active_from < active_until < final_expiry`.

A mint **SHOULD** at all times have at least one active keyset per unit whose `active_until` is `null` or at least 30 days in the future, and **MAY** keep several keysets active at once so that a successor overlaps its predecessor.

Wallets **SHOULD** prefer the active keyset with the latest `active_until` (`null` counting as latest) for outputs that must stay spendable for a known period, for example spending conditions with a future `locktime` ([NUT-10][10], [NUT-11][11], [NUT-14][14]).

A mint **MUST NOT** accept blank change outputs ([NUT-08][08]) on a keyset whose `final_expiry` is earlier than the melt quote `expiry` ([NUT-05][05]).
  ...
  "keysets": [
    {
      ...
      "active_from": <int|null>,
      "active_until": <int|null>,
...
Here, `id` is the keyset ID, `unit` is the unit string (e.g. "sat") of the keyset, `active` indicates whether new ecash can be minted with this keyset, and `input_fee_ppk` is the fee (per thousand units) to spend one input spent from this keyset. If `input_fee_ppk` is not given, we assume it to be `0`. `active_from`, `active_until` and `final_expiry` are optional unix epoch timestamps described above.

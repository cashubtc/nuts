...
Three keyset ID versions are defined: V3 (version byte `02`, current), V2 (`01`) and V1 (`00`), both deprecated and specified in [legacy NUT-02](legacy/02.md).

The `unit` string **MUST** match `[a-z0-9_-]+`.

#### Keyset ID V3

V3 keyset IDs are 33 byte hex strings with version byte `02`. V3 identifies keysets that use the BLS12-381 blinding protocol defined in [Pairing-based BDHKE (BLS12-381)](00.md#pairing-based-bls-blind-signatures).

Keyset IDs are derived from public data. The preimage is built over raw bytes:

```
keys = for each key of the keyset (sorted by amount, ascending):
         len32(amount) || amount
         || len32(pubkey) || pubkey

preimage = len32(keys) || keys
           || len32(unit) || unit
           || len32(fee)  || fee
```

The keyset ID is the hex-encoded SHA256 hash of the preimage, prefixed with the keyset ID version byte `02`.

Where:

- `||` denotes byte concatenation and `len32(x)` is the 32-bit (4-byte) big-endian length of the byte array `x` in bytes.
- `amount` and `fee` are canonical minimal big-endian bytes (e.g. `0` -> empty byte array, `1` -> `0x01`, `256` -> `0x0100`).
- `pubkey` is the compressed BLS12-381 G2 public key as raw bytes (96 bytes).
- `unit` is the UTF-8 encoded unit string, matching `[a-z0-9_-]+`.
- `fee` is the keyset's `input_fee_ppk`; when `input_fee_ppk` is omitted, null, or `0`, `fee` is the empty byte array. The length prefix is always present, so an empty `fee` contributes `len32(fee) || fee` = `00 00 00 00`.

An example implementation in Python:

```python
def derive_keyset_id_v3(keys: Dict[int, G2PublicKey], unit: str, input_fee_ppk: Optional[int]) -> str:
    ...
    # v.serialize() returns the compressed G2 bytes (96 bytes).
    keys_bytes = b"".join(
        [framed(minimal_be(k)) + framed(v.serialize()) for k, v in sorted_keys.items()]
    )
    preimage = (
        framed(keys_bytes)
        + framed(unit.encode("utf-8"))
        + framed(minimal_be(input_fee_ppk or 0))
    )
    return "02" + hashlib.sha256(preimage).hexdigest()
```
...
[11]: legacy/11.md
[12]: legacy/12.md
[TokenV3]: legacy/00.md#v3-tokens

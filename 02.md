...
A keyset **MAY** announce when it becomes active and when it stops being active with the optional `active_from` and `active_until` unix epoch timestamps. Neither field is part of the keyset ID, so a mint can add or extend them without rotating.

...
Unless intending to cease operations, a mint **MUST** at all times have at least one active keyset per unit whose `active_until` is `null` or at least 30 days in the future, and **MAY** keep several keysets active at once so that a successor overlaps its predecessor.

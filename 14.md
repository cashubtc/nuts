...
> [!NOTE]
> This NUT applies to keysets with version byte `00` or `01`. On keysets with version byte `02` or later, hash locks are expressed as [NUT-10 nutroot secrets](10.md#nutroot-secrets-v3-keysets) (`hashlock` leaves), which always require a signature alongside the preimage. A v3 hashlock used as an externally observable HTLC **MUST** carry `disclosure` mode `0x01`, which discloses its exercised witness through [NUT-07][07] and [NUT-17][17]; a hashlock without `disclosure` is private to the mint and spender.

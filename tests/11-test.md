...
Derived `SigAllSigningPackage` JSON. Note the representation change: `Proof.witness` above is a serialized JSON string, while the package's `witness` is the decoded object. The `secret` is carried over unchanged.

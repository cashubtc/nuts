...
> This document specifies keysets with version byte `00` or `01`. Keysets with version byte `02` or later lock every mint quote and sign it as a transaction input per [NUT-04](../04.md#nutroot-transactions-v3-keysets); quote lock keys derive per [NUT-13](../13.md#v3-message).
...
> [!NOTE]
> **Deprecated.** A wallet that uses deterministic quote lock keys derives them per [NUT-13](../13.md#v3-message) (`derivation_type_byte` `0x04`) and **MUST NOT** issue new quotes with keys from the path below; random keys remain permitted. The path is retained to recover keys that legacy wallets created.

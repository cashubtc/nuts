...
For asynchronous melt operations, the wallet should periodically check the quote state:

```http
GET https://mint.host:3338/v1/melt/quote/{method}/{quote_id}
```

The response will include the current state:

- `"PENDING"` - Payment is being processed
- `"PAID"` - Payment completed successfully
- `"UNPAID"` - Payment failed or was not attempted

#### WebSocket Notifications (Optional)

Mints may optionally provide websocket endpoints for real-time notifications of state changes. This allows wallets to receive immediate updates without polling. The specific websocket implementation is mint-dependent and should be documented by the mint operator.

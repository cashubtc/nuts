...
2. The mint responds with one or more quotes that include a `quote` id and an `amount` demanded in the requested unit. Most methods return a single quote; method-specific NUTs can define multiple quotes when a payment method exposes multiple execution options for the same request.
...
The mint `Bob` usually responds with a quote that includes some common fields for all methods:
...
Method-specific NUTs may define a different response shape. In particular, [NUT-XX][XX] can return multiple melt quotes for a single request, where each quote represents a different absolute fee and confirmation estimate for the same payment.
...
The mint responds with the same structure as the selected initial quote response. If the quote creation response returned multiple quotes, the wallet checks the state of the quote it selected by using that quote's `quote` id.

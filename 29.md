...
To prevent ambiguity and provide domain separation, the message `msg_to_sign` for `quotes[i]` is constructed as a byte string using the following pattern:

```
msg_to_sign = b"Cashu_MintQuoteSig_v1"                 // DST
...
- `len32(x)` is the 32-bit (4-byte) big-endian length of the following data `x`
- `amount_i` is the integer amount encoded as canonical minimal big-endian bytes (e.g. `0` = empty byte array, `1` = `0x01`, `256` = `0x0100`)
- `B_i` is the raw byte representation of the blinded message (e.g., 33-byte compressed secp256k1 public key or 48-byte BLS public key), decoded from the hex string in the request.

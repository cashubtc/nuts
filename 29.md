...
The batch is one transaction with every quote as an input, so `signatures[i]` is quote `i`'s witness over its own [NUT-10 input digest](10.md#the-transaction-transcript), whose shared transcript covers all quote inputs and all `outputs`, instead of the NUT-20 message above.

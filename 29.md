...
#### Nutroot transactions (v3 keysets)

A batch request with any output on a v3 keyset follows this section, and all its `outputs` **MUST** share that keyset ([NUT-04](04.md#nutroot-transactions-v3-keysets)). Every quote in the batch **MUST** be locked; a batch containing an unlocked quote **MUST** be rejected.

The batch is one transaction with every quote as an input, so `signatures[i]` is quote `i`'s witness over its own [NUT-10 input digest](10.md#the-transaction-transcript), whose shared transcript covers all quote inputs and all `outputs`, instead of the NUT-20 message above. Each quote input commits its `quote_amounts` entry, or the quote's mintable amount when `quote_amounts` is absent.
...
[06]: 06.md
[20]: legacy/20.md
[20-msg-agg]: legacy/20.md#message-aggregation

...
The batch is one transaction with every quote as an input, so `signatures[i]` is quote `i`'s witness over the single [NUT-10 transaction digest](10.md#the-transaction-transcript) covering all quote inputs and all `outputs`, instead of the NUT-20 message above.

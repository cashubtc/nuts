...
Every `Proof` in the `inputs` requires a signature over the serialized swap transaction from the public key `secret = SecpPubKey(xpriv)` (see [NUT-00][00] Transaction signatures). To provide a valid signature the wallet must concatenate the `secret` and `C` fields of all `Proofs` (inputs) with the `amount` and `B_` fields of all `BlindedMessages` (outputs, see [NUT-00][00]) to a single serialized message string in the order they appear in the transaction. 

If a swap transaction has `n` inputs and `m` outputs, the message to sign becomes:

```
msg_to_sign = secret_0 || C_0 || ... || secret_n || C_n || amount_0 ||  B_0 || ... || amount_m || B_m
```

> TODO: decide which fields to include, why not keyset_id for inputs and outputs, and amounts for inputs as well?

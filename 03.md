...
For proofs on keysets with version byte `02` or later, a swap is a [transaction](10.md#the-transaction-transcript): every v3 input **MUST** carry a witness over the transaction digest per [NUT-10][10]. Verification is per input: keyset versions mix freely, each input under its own rules, and `SIG_ALL`/`sigflag` do not apply to v3 inputs.

...
Bob:
r = HMAC-SHA256(key=a, data="Cashu_DLEQ_R_v1" || A || B' || C' || 0x00) mod n # Deterministic nonce
R1 = r*G
R2 = r*B'
e = hash(R1,R2,A,C')
s = (r + e*a) mod n
...
where:
  ...
  0x00 = nonce counter byte; increment and retry if r == 0 (max 256)
```

> [!CAUTION]
>
> Mints **SHOULD** use a deterministic nonce (`r`), because reuse across different challenges leaks the private key immediately (`a = (s₁ - s₂) · (e₁ - e₂)⁻¹ mod n`).
> Mints that use random nonces **MUST** source them from a cryptographically secure RNG. See the [test vectors][tests].

...
Bob:
r = HMAC-SHA256(key=SHA256(a), data="Cashu_DLEQ_R_v1" || A || B' || C' || 0x00) mod n
R1 = r*G
R2 = r*B'
e = hash(R1,R2,A,C')
s = r + e*a
...
```
r = HMAC-SHA256(key=SHA256(a), data="Cashu_DLEQ_R_v1" || A || B' || C' || ctr) mod n
```

`A`, `B'`, and `C'` are uncompressed SEC1 (65 bytes each). `ctr` starts at `0x00` and increments if `r == 0` (max 256 attempts).

...
Caution: If the mint does not support spending conditions or a specific `kind` of spending condition, proofs may be treated as regular anyone-can-spend tokens. Applications need to make sure to check whether the mint supports a specific `kind` of spending condition by checking the mint's [info][06] endpoint.
...
- Outputs referring to the `BlindedMessages` with new unlock conditions to which the `Proofs` are spent
...
New `Secret`s of the outputs to which the inputs are spent are provided as `BlindedMessages` which means that they are blind-signed and not visible to the mint until they are actually spent.

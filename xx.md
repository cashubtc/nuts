...
For real-time mirroring, the firehose is exposed as a WebSocket at the mint's `/v1/firehose/ws` path, reusing the JSON-RPC framing of [NUT-17][17].

```
https://mint.host:3338/v1/firehose/ws
```

A delegate subscribes with a `WsRequest` carrying the `subscribe` method and a single `SubscriptionKind` value `spent_feed`:
...
`subId` is a UUID v7 generated by the subscriber and `filters` is empty (the feed is not filtered). The mint replies with a [NUT-17][17] `WsResponse` and then streams every `SpentEvent` as a `WsNotification` as proofs are spent:
...
A delegate that has been offline, or that is bootstrapping its mirror, fetches missed events over HTTP:

```http
GET https://mint.host:3338/v1/firehose?since=<int>&limit=<int>
```

- `since` (optional, default `0`) returns events with `seq >= since`.
- `limit` (optional) caps the number of events returned; the mint MAY impose its own maximum.

The mint responds with a `FirehoseResponse`:
...
- `events` is the ordered slice of `SpentEvent`s, ascending by `seq`.
- `latest` is the highest `seq` the mint has published, so the consumer knows whether more pages remain.
...
- `firehose` gives the WebSocket (`ws`) and HTTP catch-up (`http`) base URLs for this mint's spent-proof feed.
...
"nuts": {
  "xx": {
    ...
    "firehose": {
      "ws": "https://bob.example/v1/firehose/ws",
      "http": "https://bob.example/v1/firehose"
    },
    "delegate_for": [

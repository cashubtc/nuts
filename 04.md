...
For that, a wallet sends a `POST /mint?payment_hash=<hash>` request with a JSON body to the mint. The body **MUST** include `BlindedMessages` (see [NUT-00][00]) that are worth a maximum of `<amount_sat>`. If successful (i.e. the invoice has been previously paid and the `BlindedMessages` are valid), the mint responds with `Promises` (see [NUT-00][00]).

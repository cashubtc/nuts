...
The items are first passed through the pseudorandom function SipHash, which takes a 128-bit key `k` and a variable-sized byte vector and produces a uniformly random 64-bit output. Implementations of this NUT **MUST** use the SipHash parameters `c = 2` and `d = 4`.

The 64-bit SipHash outputs are then mapped uniformly over the desired range by multiplying with `F` and taking the top 64 bits of the 128-bit result. This algorithm is a faster alternative to modulo reduction, as it avoids the expensive division operation while still retaining a fair mapping property.

> [!NOTE] > $\biggl\lfloor\frac{F \cdot \text{SipHash}(x)}{2^{64}}\biggr\rfloor \neq \text{SipHash}(x) \bmod F$.
...
Users **MAY** -on necessity- query `GET v1/filter/spent/{keyset_id}` to get the GCS filter that encodes the `Y` values of all the spent ecash from `keyset_id`.
...
- `k` is the key for the SipHash function. If `null`, then the client assumes `k = "00000000000000000000000000000000"`.

 ...
 * `GET v1/filter/spent/{keyset_id}` to get the GCS filter that encodes the `Y` values of all the spent ecash from `keyset_id`.
 * `GET v1/filter/issued/{keyset_id}` to get the GCS filter that encodes the `B_` values of all the blind signatures from `keyset_id`.
...
Each invidivual look-up has $1 - \frac{1}{M}$ chance of being a _true positive_. We can consider a bulk test as $x$ independent look-ups, so the chance that at _all_ of them are true positives is $\bigl(1 - \frac{1}{M}\bigr)^x$.

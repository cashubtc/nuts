...
```json
{
  ...
  "options": {
    "amountless": true,
    "preimage_guaranteed": true
  }
}
```

`preimage_guaranteed` indicates that the mint guarantees to include the `payment_preimage` field in a successful (`"PAID"`) melt response for this method-unit pair. When set to `true`, the mint **MUST** return `payment_preimage` in every response with `"state": "PAID"`. When `false` or omitted, the mint **MAY** omit `payment_preimage` from `"PAID"` melt responses. Wallets use this option to determine, before any payment is attempted, whether a melt can be used with protocols that require the payment preimage (such as [L402](https://github.com/lightninglabs/L402)).

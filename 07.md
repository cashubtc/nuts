...
- `witness` is the serialized witness data that was used to spend the `Proof`. On keysets with version byte `00` or `01`, it is present if the token has a [NUT-10][10] spending condition that requires a witness, such as P2PK ([NUT-11][11]) or HTLCs ([NUT-14][14]). On keysets with version byte `02`, every input signs ([NUT-10](10.md#the-signing-rule)), so it is present for every spent proof.
- `digest` is the transaction digest the witness's signatures cover (32 bytes hex, [NUT-10](10.md#the-transaction-transcript)): present for spent proofs on keysets with version byte `02`, whose witnesses sign the digest rather than the secret, and absent otherwise. A mint returning a version `02` `witness` MUST also return its `digest`, since the witness verifies only against it.

...
Where `proofs` is a list (array) of `Proof`s (see [NUT-00][00]). `Alice` **CAN** provide a full `Proof` but **MUST** provide at least the `secret` (which is the recommended way).

...
```json
{
  ...
  "taproot": TaprootOption <optional>,
}
```
...
- `taproot`: The required [taproot locking](#taproot-locking-v3-keysets) (v3 keysets)
...
### Taproot locking (v3 keysets)

For payments on keysets with version byte `02` or later, the `taproot` field replaces `nut10`. Its presence is a request for receiver-keyed (P2BK) outputs: every output's secret must be derived from `k` using [NUT-28][28], never locked to `k` verbatim and never sent as a bearer proof, and the payee verifies this on receive. The `TaprootOption` is defined as follows:
...
`k` **MAY** be the [NUMS point](10.md#bare-empty-tweaked-and-script-only-secrets) `H` to request proofs spendable through their leaves only. The payer then derives `K = H + r*G` with a fresh `r` per output instead of blinding `k` at slot 0, and returns `r` in spend info so the payee can verify `K - r*G == H`. The offset supplies per-payment uniqueness, so such a request needs no `b` entry and the requested tree is reproduced unchanged.

The payment flow:

1. The payer picks a fresh ephemeral keypair `(e, E)` per output and blinds `k` at slot 0 into the internal key `K` ([NUT-28][28]); a NUMS `k` is offset by a fresh `r` instead ([NUT-10](10.md#bare-empty-tweaked-and-script-only-secrets)).
...
3. The payer computes the tree root and the tweaked secret `P = K + t*G` ([NUT-10](10.md#taproot-secrets-v3-keysets)), swaps to an output with secret `P`, and sends the proof with spend info `E`, the transmitted tree, and `K`.
4. The payee derives the internal key from `E` and her static key (or verifies a NUMS offset with `K - r*G == H`), runs the [receive-time cascade](10.md#receive-time-verification), and **MUST** verify the disclosed tree is exactly the requested tree (see below).

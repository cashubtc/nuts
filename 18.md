...
`k` **MAY** be the [NUMS point](10.md#bare-empty-tweaked-and-script-only-secrets) to request proofs spendable through their leaves only. The payer then uses it **verbatim** as the internal key (a blinded NUMS is unverifiable), and such a request **MUST** tag at least one leaf key blind-me in `b`: the blinded leaf key is what makes each payment's secret unique ([NUT-10](10.md#key-uniqueness)). A NUMS request without one is malformed and **MUST** be refused.

The payment flow:

1. The payer picks a fresh ephemeral keypair `(e, E)` per output and blinds `k` at slot 0 into the internal key `K` ([NUT-28][28]); a NUMS `k` is used verbatim instead.
...
4. The payee derives the internal key from `E` and her static key (or checks a NUMS `K` byte-for-byte), runs the [receive-time cascade](10.md#receive-time-verification), and **MUST** verify the disclosed tree is exactly the requested tree (see below).

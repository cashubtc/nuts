...
- type: `ndef`
- target: `<empty>`
- tags: `[["r", "text"], ["p", "<hex-compressed-pubkey>"]]`

The recipient of the payment is a NDEF (NFC Data Exchange Format) tag or any device that uses HCE (Host Card Emulation) to simulate a NDEF tag.
The payer is agnostic to this distinction.

The payer serializes the payload into a base-64 encoded TokenV4 ([NUT-00](00)) and constructs a [`NDEFMessage`][ndefmessage] consisting of one single [`NDEFRecord`][ndefrecord] containing the entire token.

The `r` tag specifies the [`NDEFRecordType`][ndefrecordtype] that must be used for the record. Currently, `"text"` is the only option. The `p` tag lists the hex of the public key in compressed form, which the sender must lock the ecash to ([NUT-11](11)).

To start the transaction, the sender approaches their device to the recipient's device. The sender's wallet is then responsible for detecting the tag and transmitting the payload to it following NDEF protocol.
...
[ndefrecordtype]: https://developer.mozilla.org/en-US/docs/Web/API/NDEFRecord/recordType

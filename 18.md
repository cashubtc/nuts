...
The `r` tag specifies the [`NDEFRecordType`][ndefrecordtype] that must be used for the record. Currently, `"text"` is the only option. The `p` tag lists the hex of the public key in compressed form, which the sender must lock the ecash to ([NUT-10](10)).

...
The `nut10` field specifies the payee's requested locking condition as a `NUT10Option` object. Its elements are derived from NUT-10's [well-known secret](legacy/10.md#well-known-secret). The `NUT10Option` is defined as follows:
...
> [!IMPORTANT]
> The payee must validate the incoming tokens themselves in order to decide whether they can accept the payment. This includes checking the mint's signature (the [pairing equality](00.md#pairing-based-bls-blind-signatures) on v3 keysets, the DLEQ proof on earlier ones) and whether the token includes a long-enough timelock to satisfy the payee.
...
1. The payer picks a fresh ephemeral keypair `(e, E)` per output and blinds `k` at slot 0 into the internal key `K` ([NUT-28][28]); a NUMS `k` is offset by a fresh `u` instead ([NUT-10](10.md#the-internal-key)), with an ephemeral only when `b` tags keys to blind.
...
Here, `id` is the payment id (corresponding to `i` in request), `memo` is an optional memo to be sent to the receiver with the payment, `mint` is the mint URL from which the ecash is from, `unit` is the unit of the payment, and `proofs` is an array of proofs (see [NUT-00][00]).

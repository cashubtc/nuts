...
- `nf`: Whether the requested amount is net of input fees (see [Input fees](#input-fees))
...
If `sm` is set, the payer MUST send ecash from a mint that supports at least one of the listed payment methods. Each `SupportedMethod` is defined as:
...
Received proofs cost the receiver an input fee when they are later swapped or melted (see [NUT-02][02]). If `nf` is `true`, the requested amount is net of input fees: the payer MUST select proofs such that

`sum(proofs) - input_fee(proofs) >= a + mf (where applicable)`

with `input_fee` computed from the sending mint's keyset `input_fee_ppk` values as described in [NUT-02][02]. If `nf` is `false` or omitted, the proofs need only sum to the requested amount plus any applicable `mf`.

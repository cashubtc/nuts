...
- `ms`: A flag indicating whether the mint list is strict; defaults to `true`- `sm`: A list of supported methods (e.g. `"bolt11"`, `"bolt12"`, `"on-chain"`) that the mint from which the payer sends ecash MUST support.

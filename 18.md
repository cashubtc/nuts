...
4. The payee derives the internal key from `E` and her static key (or checks a NUMS `K` byte-for-byte), runs the [receive-time cascade](10.md#receive-time-verification), and **MUST** verify the disclosed tree honors `l` and `b`, matching blinded keys by value ([NUT-28][28]).

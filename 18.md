...
For payments on keysets with version byte `02` or later, the `nutroot` field replaces `nut10`. Its presence is a request for receiver-keyed (P2BK) outputs: every output's secret must be derived from `k` using [NUT-28][28], never locked to `k` verbatim and never sent as a bearer proof, and the payee verifies this on receive. The `NutrootOption` is defined as follows:

...
- `ms`: Whether the mint list is strict. Defaults to `true` when `m` is set
- `fr`: Additional fee reserve, in the request unit, that the payer MUST add when paying from a mint outside a non-strict mint list
- `sm`: A list of supported methods (e.g. `"bolt11"`, `"bolt12"`, `"onchain"`) that the mint from which the payer sends ecash MUST support.
...
If `m` is set and `ms` is not present, or is `true`, the mint list is strict: the sender MUST only send proofs from these mints, and the receiver SHOULD ignore payments sent from other mints.

If `m` is set and `ms` is `false`, the mint list is preferred: the receiver accepts payments from other mints, but the payer SHOULD use a mint in `m` when possible. If the payer sends ecash from a mint outside `m` and `fr` is set, the payer MUST add `fr` to the requested amount as a fee reserve.

If `m` is not set, `fr` and `ms` SHOULD be ignored.

If `sm` is set, the payer MUST send ecash from a mint that supports at least one of the listed payment methods.

...
If `m` is set and `mp` is `true`, the mint list is preferred: the receiver accepts payments from other mints, but the payer SHOULD use a mint in `m` when possible. If the payer sends ecash from a mint outside `m` and `fr` is set, the payer MUST add `fr` to the requested amount as a fee reserve.

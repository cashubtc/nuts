...
If `m` is set and `ms` is not present, or is `true`, the mint list is strict: the sender MUST only send proofs from these mints, and the receiver MAY ignore payments sent from other mints.

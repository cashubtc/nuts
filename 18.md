...
- `fr`: Additional fee required, in the request unit, that the payer MUST add when paying from a mint outside a non-strict mint list
...
If `m` is set and `mp` is `true`, the mint list is preferred: the receiver accepts payments from other mints, but the payer SHOULD use a mint in `m` when possible. If the payer sends ecash from a mint outside `m` and `fr` is set, the payer MUST add `fr` to the requested amount as an additional fee.

If `m` is not set, `fr` and `mp` SHOULD be ignored.

...
- **The slot map.** Slot `0` is the base key: the secret itself for a bare output, the internal key under a tree. Slots `1..` are the entries of `keys` fields, walking the spend-info tree list in transmitted order, keys within each leaf in order. The same static key at two positions gets two unrelated blinded keys, since the slot index is in the tweak preimage.
- **Slot `0` is always blinded**: a receiver-keyed send _is_ that blinding. The one exception is a [NUMS point](10.md#bare-empty-tweaked-and-script-only-secrets) base, which travels verbatim: nobody holds its scalar for the receiver's half of the ECDH, so a blinded NUMS is unverifiable. Slot indices are unchanged either way.
...
- **One fresh ephemeral per output, no exceptions**, including receiver-keyed quote locks ([NUT-04](04.md#taproot-transactions-v3-keysets)). The `SIG_ALL` exception above does not exist for v3 keysets: an index collision under a shared `e` reproduces the base key, hence a colliding `C` ([NUT-10](10.md#key-uniqueness)).

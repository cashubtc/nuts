...
- **Receivers match by value, never by position.** The root does not commit the transmitted leaf order ([NUT-10](10.md#receive-time-verification)). The receiver derives its candidate key for each slot in `1..N` (`N` = number of enumerated keys) and takes whichever slot reproduces a key in the tree. (A canonical leaf order is impossible: blinding rewrites the leaf bytes, so an order derived from them exists only after the slots it would decide.)
...
- **One fresh ephemeral per output, no exceptions**, including receiver-keyed quote locks ([NUT-04](04.md#nutroot-transactions-v3-keysets)). The `SIG_ALL` exception above does not exist for v3 keysets: an index collision under a shared `e` reproduces the internal key, hence a colliding `C` ([NUT-10](10.md#key-uniqueness)).

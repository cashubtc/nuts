...
- **At most 255 enumerated slots per secret**: slot `0` plus at most 254 leaf keys. Both sides **MUST** enforce the cap: a 256th key would derive slot `0`'s tweak, so an over-long tree is refused rather than truncated onto a colliding index.

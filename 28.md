...
On keysets with version byte `02` or later, P2BK's cryptographic core applies one layer down, to [NUT-10 nutroot secrets](10.md#nutroot-secrets-v3-keysets): the same domain separator, the same per-key ECDH secret `Zx = x(eP)`, the same one-byte slot index with the `0xff` retry, the same blinding `P' = P + rᵢG`. What changes:

- **The slot map.** Slot `0` is always the internal key `K`, which for a bare output is the secret itself. Slots `1..` are the entries of `keys` fields, walking the spend-info tree list in transmitted order, keys within each leaf in order. The same static key at two positions gets two unrelated blinded keys, since the slot index is in the tweak preimage.
- **Slot `0` is always blinded**: a receiver-keyed send _is_ that blinding. The one exception is a [NUMS](10.md#bare-empty-tweaked-and-script-only-secrets) internal key, which is not ECDH-blinded: nobody holds its scalar for the receiver's half of the DH, so a blinded NUMS would be unverifiable. It takes its per-proof uniqueness from the NUMS offset `K = H + u*G` instead. Slot indices are unchanged either way.
- **Receivers match by value, never by position.** The fold sorts, so the root commits the leaf set and any transmitted order reconstructs it ([NUT-10](10.md#receive-time-verification)). The receiver derives its candidate key for each slot in `1..N` (`N` = number of enumerated keys) and takes whichever slot reproduces a key in the tree.
- **At most 256 enumerated slots per secret**, exactly the one index byte: slot `0` plus at most 255 leaf keys. Both sides **MUST** enforce the cap: a 256th leaf key would wrap onto slot `0`'s tweak, so an over-long tree is refused rather than truncated onto a colliding index.
- **One fresh ephemeral per output, no exceptions**, including receiver-keyed quote locks ([NUT-04](04.md#nutroot-transactions-v3-keysets)). The `SIG_ALL` exception above does not exist for v3 keysets: an index collision under a shared `e` reproduces the internal key, hence a colliding `C` ([NUT-10](10.md#key-uniqueness)). A NUMS output with no blind-me keys blinds nothing and carries no ephemeral at all ([NUT-18](18.md#nutroot-locking-v3-keysets)).
- **The negated-derivation branch retires.** v3 keys are compressed points with explicit parity, so `k = (p + rᵢ) mod n` is always correct once wallets normalize x-only imports at the boundary (store `n − d` when `dG` has odd Y).
- **Whether a leaf key is blinded is its owner's choice**, not the proof constructor's. The blind-me / use-verbatim tag travels on the key's delivery channel (eg a payment request's `b` set, [NUT-18](18.md#nutroot-locking-v3-keysets)) and never appears in proof data; owners can check the disclosed tree honored it. Slots count every enumerated key either way.
- **The ephemeral travels in spend info**: `E` is [NUT-10's spend info field](10.md#spend-info), not `p2pk_e`. `k` and `E` are mutually exclusive there, and a derived private key **MUST NOT** be re-gifted as a bearer key.

See the [test vectors][tests] for a worked slot derivation and value match.

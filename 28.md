...
Mints implementing NUT-28 **MUST** enforce that [NUT-03][03] swap requests involving conditional keysets reject cross-keyset operations. This prevents redemption of conditional tokens without oracle attestation.

Within [NUT-03][03]:

- Swaps within the same conditional keyset (trading) work as normal
- Swaps within a regular keyset work as normal, including cross-keyset swaps between regular keysets (e.g., for keyset rotation)
- Any swap where inputs or outputs use a conditional keyset **MUST** require all inputs and all outputs to share the same keyset ID

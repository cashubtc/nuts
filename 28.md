...
On keysets with version byte `02` or later, P2BK's cryptographic core applies one layer down, to [NUT-10 taproot secrets](10.md#taproot-secrets-v3-keysets): the same domain separator, the same per-key ECDH secret `Zx = x(eP)`, the same one-byte slot index with the `0xff` retry, the same blinding `P' = P + rᵢG`. What changes:
...
- **One fresh ephemeral per output, no exceptions**, including receiver-keyed quote locks ([NUT-04](04.md#taproot-transactions-v3-keysets)). The `SIG_ALL` exception above does not exist for v3 keysets: an index collision under a shared `e` reproduces the internal key, hence a colliding `C` ([NUT-10](10.md#key-uniqueness)).
...
- **Whether a leaf key is blinded is its owner's choice**, not the proof constructor's. The blind-me / use-verbatim tag travels on the key's delivery channel (eg a payment request's `b` set, [NUT-18](18.md#taproot-locking-v3-keysets)) and never appears in proof data; owners can check the disclosed tree honored it. Slots count every enumerated key either way.

...
| 0x08 | -                | -         | Legacy: pre-v3 NUT-10 spending conditions, still emitted as a transition measure ([legacy NUT-26](legacy/26.md)) |
...
| 0x0b | nutroot          | sub-TLV   | Nutroot locking option (corresponds to `nutroot` in JSON)                                                        |
...
### Nutroot Sub-TLV (Tag 0x0b)

The [NUT-18][18] `nutroot` locking option is encoded as a nested TLV structure:

| Sub-Tag | Field        | Type  | Description                                                                     |
| ------- | ------------ | ----- | ------------------------------------------------------------------------------- |
| 0x01    | receiver_key | bytes | Static receiver key, 33-byte compressed point (corresponds to `k` in JSON)      |
| 0x02    | leaf         | bytes | Serialized condition leaf (repeatable, in request order; the `l` array in JSON) |
| 0x03    | blind_key    | bytes | Leaf key tagged blind-me, 33 bytes (repeatable; the `b` array in JSON)          |

`receiver_key` is required. Keys and leaves are raw bytes, re-encoded as lowercase hex in JSON. A `leaf` value is exactly the serialized leaf of [NUT-10][10], so [NUT-18][18]'s byte-for-byte reproduction rule survives the round trip unchanged; repeated `leaf` tags preserve request order.

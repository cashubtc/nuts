...
| 0x09 | mint_strict       | u8        | Mint list strict flag: 0=false, 1=true; defaults to 1 if absent (corresponds to `ms` in JSON) |
| 0x0a | fee_reserve       | u64       | Additional fee reserve for payments from non-preferred mints (corresponds to `fr` in JSON)    |
| 0x0b | supported_methods | string    | Supported payment method (repeatable for multiple methods, corresponds to `sm` in JSON)       |

...
| 0x0a | fee_required     | u64       | Additional fee required for payments from non-preferred mints (corresponds to `fr` in JSON)       |
| 0x0b | supported_method | sub-TLV   | Supported payment method with optional fee (repeatable, corresponds to `sm` in JSON)              |
...
### Supported Method Sub-TLV (Tag 0x0b)
...
The per-method fee (`mf`, sub-tag 0x02) is independent of the top-level fee required for non-preferred mints (`fr`, tag 0x0a): if the payer sends from a non-preferred mint (outside `m`, `mp` = true) **and** relies on an `sm` method that carries a fee, the payer MUST add both the `fr` and `mf` fees.

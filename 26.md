...
- Unblinded pubkey: P.
  ...
  - For BIP340 Schnorr/x-only contexts (e.g., Nostr), P is an x-only 32-byte pubkey (hex) with even-Y parity implied by BIP340.
- Blinding scalar: r with 1 ≤ r ≤ n − 1.
- Blinded pubkey: P' = P + r·G. Encoding matches P: compressed SEC1 (33-byte hex) for SEC1 keys; x-only (32-byte hex, even-Y) for BIP340 contexts.

...
Melting is a two-step process: 
1. Request a melt quote for an external payment.
2. Provide Inputs and execute the melt.
...
Decoy shaping rules:
- If the wallet has fewer than two Inputs, it SHOULD obtain zero-amount Notes from [AC-06](AC06) and include them as decoy Inputs.
...
- Let `fees = sum(input_fees)` per [AC-02](AC02).
- Let `amount = quote_amount`.
- The BalanceProof MUST verify with `delta = amount + fees` (i.e., `inputs - outputs == amount + fees`).
## 6. Mint Verification
...
4) Nullifier checks: reject if already seen. (one per Input)

...
Dependencies: AC-00, AC-01, AC-02
```
...
Decoy shaping rules:
- If the wallet has fewer than two Inputs, it **MUST** obtain zero-amount Notes from [AC-06](AC06) and include them as decoy Inputs.
...
- For each Input, extract `script`/`witness` if present and verify per [NUT-11](11) or [NUT-14](14) or any other future spending condition.
- Reject on spending conditions failure.

3) Nullifier checks
- Derive nullifier from `input.randomized_commitments.Cv`.
- Reject if nullifier already seen (double-spend prevention).
...
- With per-input fees (see [AC-02](AC02)), `delta = sum(input_fees)`.
...
If verification succeeds, the wallet combines `UnsignedNote`, `MAC`, and `IssuanceProof` into a `Note` ([AC-00](AC00)).

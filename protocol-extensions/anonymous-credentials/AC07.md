...
- keyset_id_bytes = 8‑byte raw keyset id (big‑endian)
- counter_bytes = 4‑byte unsigned big‑endian counter
...
- tag := SerializeScalar(K_0) as 32‑byte hex (lowercase, no 0x). This becomes UnsignedNote.tag (64‑hex).
- amount_blind := SerializeScalar(K_1) as 32‑byte hex; pass as the optional blinding factor when constructing AmountAttribute.
- script_blind := SerializeScalar(K_2) as 32‑byte hex; pass as the optional blinding factor when constructing ScriptAttribute.

Rust sketch:
```rust
fn derive_scalar_hex(path: &str) -> String { /* BIP32 hardened derivation → 32B → hex */ }
...
- Keeping derivations keyset‑scoped avoids cross‑mint/keyset correlation; wallets MUST derive `keyset_id_int` per AC‑01/02.
...
Add cross‑wallet test vectors once a reference implementation exists.

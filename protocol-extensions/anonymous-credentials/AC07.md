...
### 2.2 v2 — HMAC-SHA512 KDF (no BIP32)

For version 01 keysets, derive each component with an HMAC‑SHA512 keyed by the BIP39 seed. This mirrors the NUT‑13 v2 KDF and adds a component byte to obtain three distinct outputs.
...
- label = ASCII string "Cashu_KDF_HMAC_SHA512"

Compute for each c:

```
raw = HMAC_SHA512( key=seed,
                   data = label || keyset_id_bytes || counter_bytes || c_bytes )
```

Obtain a 32‑byte scalar by reducing the left 32 bytes mod n (or the full 64‑byte output mod n):

```
scalar_c = bytes_to_scalar(raw[0..32]) mod n

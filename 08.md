...
`Bob` then signs these blank outputs (now with the imprinted amounts) and thus generates `BlindSignature`s. `Bob` **MUST** sign them with the keyset of the blank outputs, even if that keyset has been inactivated since the request was accepted (see [NUT-02][02]). `Bob` then returns a payment status to the wallet, and, in addition, all blind signatures it generated for the overpaid fees.

Importantly, while `Bob` does not necessarily return the same number of blind signatures as it received blank outputs from `Alice` (since some of them may be of value 0), `Bob` **MUST** imprint the amounts into the leading blank outputs, **MUST** return all blind signatures with a value greater than 0 in the same order as the blank outputs were received, and **MUST** omit all blind signatures with value 0.

For example, if `Bob` receives 10 blank outputs but the overpaid fees only occupy 4 blind signatures, `Bob` imprints the amounts into the first 4 blank outputs and returns only those 4 blind signatures, omitting the remaining 6 with value 0.

Due to the well-defined order of the returned blind signatures, `Alice` can map the blind signatures returned from `Bob` to the blank outputs it provided so that she can further apply the correct unblinding operations on them.
...
The field `change` is an array of `BlindSignatures` that account for the overpaid fees. Notice that the amount has been changed by the mint. `Alice` **MUST** unblind each `BlindSignature` with the keys of the keyset named by its `id`, as described in [NUT-00][00] and as she does in [NUT-04][04] when minting new tokens, and **MUST NOT** reject change because that keyset is no longer `active`. After generating the `Proofs`, `Alice` stores them in her database.
